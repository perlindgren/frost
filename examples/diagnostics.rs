...
//! frame rate and the window size as a text line in the window's top-left
//! corner, set in `assets/fonts/Leofont-Regular.ttf`. The integration is
//! one struct in the demo state and one `process` call per frame — the
//! overlay appends its own text node to the scene's root on the first
//! frame, so the scene itself only carries a swaying circle under it. See
//! the module docs of `frost::Diagnostics` for the whole utility.
//! Run with:
//!
//! ```text
...
struct Demo {
    /// The diagnostics overlay, reporting the frame rate and window size.
